  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onKeyDown={handleKeyDown}>
      ...
      <div className="relative w-full max-w-md mx-4 bg-[#233138] rounded-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center gap-4 px-6 py-4 bg-[#00a884]">
          <button
          ...
          <div>
            ...
            {step === 'members' && selectedMembers.length > 0 && (
              <p className="text-sm text-white/80">{selectedMembers.length} selected</p>
            )}
          </div>
        </div>
        ...
        {step === 'members' ? (
          <>
            ...
            {selectedMembers.length > 0 && (
              <div className="px-4 py-3 border-b border-[#3b4a54] flex flex-wrap gap-2">
                {selectedMembers.map((member) => (
                  <div
                    key={member.id}
                    className="flex items-center gap-2 px-3 py-1.5 bg-[#00a884]/20 rounded-full"
                  >
            ...
            <div className="max-h-[300px] overflow-y-auto">
              {isSearching ? (
                ...
                searchResults.map((profile) => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <p className="text-xs text-[#8696a0]">{profile.email}</p>
                    </div>
                  </button>
                ))
              ) : searchQuery ? (
                ...
                <div className="py-8 text-center text-sm text-[#8696a0]">
                  <Users className="w-12 h-12 mx-auto mb-3 text-[#3b4a54]" />
            ...
            {/* Footer */}
            <div className="flex justify-end px-6 py-4 border-t border-[#3b4a54]">
              <button
                onClick={handleNext}
          ...
          <>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex justify-center">
                <div className="w-20 h-20 rounded-full bg-[#3b4a54] flex items-center justify-center">
              ...
              <div>
                <Input
                  ...
                  className="bg-transparent border-0 border-b-2 border-[#00a884] rounded-none text-[#e9edef] text-lg placeholder:text-[#8696a0] focus-visible:ring-0"
                  maxLength={50}
                />
                <p className="mt-1 text-xs text-[#8696a0] text-right">{groupName.length}/50</p>
              </div>
              ...
              <div>
                ...
                <div className="flex -space-x-2">
                  {selectedMembers.slice(0, 5).map((member) => (
                    <Avatar key={member.id} className="h-8 w-8 border-2 border-[#233138]">
                      {member.avatar_url && <AvatarImage src={member.avatar_url} />}
                      <AvatarFallback className="text-xs bg-[#00a884] text-white">
                        {getInitials(member)}
                      </AvatarFallback>
                  ...
                  {selectedMembers.length > 5 && (
                    <div className="h-8 w-8 rounded-full bg-[#3b4a54] border-2 border-[#233138] flex items-center justify-center">
                      <span className="text-xs text-[#8696a0]">+{selectedMembers.length - 5}</span>
                    </div>
                  )}
                </div>
              </div>
